(() => {
  ...
  function bindEvents() {
    $("modeInternalBtn").addEventListener("click", () => setMode("internal"));
    $("modeExternalBtn").addEventListener("click", () => setMode("external"));
  ...
  function setMode(mode) {
    ...
    $("modeExternal").classList.toggle("hidden", mode !== "external");
    $("modeInternalBtn").classList.toggle("active", mode === "internal");
    $("modeExternalBtn").classList.toggle("active", mode === "external");
  }
  ...
  function renderDocumentTable() {
    ...
    tbody.innerHTML = documents.length ? documents.map((doc) => `<tr>${DOCUMENT_COLUMNS.map((column) => `<td>${escapeHtml(doc[column] || "")}</td>`).join("")}</tr>`).join("") : `<tr><td colspan="${DOCUMENT_COLUMNS.length}">Noch keine Dokumente. Lade Beispieldokumente oder importiere JSON/TXT/CSV.</td></tr>`;
  }
