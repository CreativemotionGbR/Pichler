(() => {
  ...
  const HISTORY_COLUMNS = ["date", "source", "target_document_id", "impact_level", "measures", "status", "actions"];
  const DOCUMENT_COLUMNS = ["document_type", "title", "version", "updated_at", "status", "actions"];
  ...
  function bindEvents() {
    ...
    $("topLoadSamplesBtn").addEventListener("click", loadSampleDocuments);
    $("topExportBtn").addEventListener("click", exportCompleteBackup);
    $("topClearBtn").addEventListener("click", clearLocalData);
  ...
  function renderDashboard() {
    const highCount = history.filter((entry) => entry.impact_level === "High").length;
    const mediumCount = history.filter((entry) => entry.impact_level === "Medium").length;
    const lowCount = history.filter((entry) => entry.impact_level === "Low").length;
    ...
    $("metricHighImpact").textContent = String(highCount);
    $("metricLowImpact").textContent = String(lowCount);
    $("metricMediumImpact").textContent = String(mediumCount);
    $("metricHighImpactSmall").textContent = String(highCount);
    ...
    $("metricUpdated").textContent = dates.length ? formatDate(dates[dates.length - 1]) : "Heute";
  }
  ...
  function renderDocumentTable() {
    ...
    const labels = { document_type: "Typ", title: "Titel", version: "Version", updated_at: "Zuletzt geändert", status: "Status", actions: "Aktionen" };
    thead.innerHTML = `<tr>${DOCUMENT_COLUMNS.map((column) => `<th>${escapeHtml(labels[column] || column)}</th>`).join("")}</tr>`;
    tbody.innerHTML = filtered.length ? filtered.map((doc) => {
      const row = { ...doc, status: documentStatus(doc), actions: "Details" };
      return `<tr>${DOCUMENT_COLUMNS.map((column) => `<td>${formatTableValue(column, row[column])}</td>`).join("")}</tr>`;
    }).join("") : `<tr><td colspan="${DOCUMENT_COLUMNS.length}">Keine passenden Dokumente gefunden.</td></tr>`;
  }
  ...
  function renderHistory() {
    ...
    const labels = { date: "Datum", source: "Quelle", target_document_id: "Dokument", impact_level: "Impact", measures: "Maßnahmen", status: "Status", actions: "Aktion" };
    thead.innerHTML = `<tr>${HISTORY_COLUMNS.map((column) => `<th>${escapeHtml(labels[column] || column)}</th>`).join("")}</tr>`;
    tbody.innerHTML = history.length ? history.map((entry) => {
      const row = { ...entry, actions: "Details" };
      return `<tr>${HISTORY_COLUMNS.map((column) => `<td>${formatTableValue(column, row[column])}</td>`).join("")}</tr>`;
    }).join("") : `<tr><td colspan="${HISTORY_COLUMNS.length}">Noch keine Änderungshistorie vorhanden.</td></tr>`;
  }

  function exportCompleteBackup() {
    downloadFile("dsgvo-local-backup.json", JSON.stringify({ documents, versions, proposals, history }, null, 2), "application/json");
  }
  ...
  function documentStatus(doc) {
    if (String(doc.version || "").toLowerCase().includes("draft")) return "In Prüfung";
    if (versions.some((version) => version.document_id === doc.document_id)) return "Aktiv";
    return doc.document_type === "TOM" ? "Extern geändert" : "Aktiv";
  }
  ...
  function formatTableValue(column, value) {
    ...
    if (column === "status" && value) return `<span class="status-badge status-${statusClass(value)}">${escapeHtml(value)}</span>`;
    if (column === "actions") return `<span class="action-link">${escapeHtml(value || "Details")}</span>`;
    if (column === "source") return `<span class="source-badge">${escapeHtml(value || "Intern")}</span>`;
    if (column === "target_document_id") return escapeHtml(value || "Änderungshistorie");
  ...
  function statusClass(value) {
    ...
    if (normalized.includes("extern") || normalized.includes("hoch")) return "danger";
    if (normalized.includes("prüfung") || normalized.includes("offen")) return "pending";
    if (normalized.includes("abgelehnt") || normalized.includes("verworfen")) return "muted";
    return "ok";
  }
